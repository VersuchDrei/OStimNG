...
namespace PapyrusLibrary {
    ...
    std::string randomScene(std::vector<RE::Actor*> actors, std::string furnitureType, std::function<bool(Graph::Node*)> condition) {
        if (Graph::Node* node = Graph::GraphTable::getRandomNode(Furniture::FurnitureTable::getFurnitureType(furnitureType), Trait::ActorConditions::create(GameAPI::GameActor::convertVector(actors)), condition)) {
